...
class FuzzyPID:
    def __init__(self, Setpoint):
        ...
        self.error = ctrl.Antecedent(np.arange(-4000, 4000, 1), 'error')
        self.delta_error = ctrl.Antecedent(np.arange(-2000, 2000, 1), 'delta_error')
        self.output = ctrl.Consequent(np.arange(0, 4000, 1), 'output')

        # Funciones de pertenencia para 'error'
        self.error['NB'] = fuzz.trapmf(self.error.universe, [-4000, -4000, -2000, -1000])
        self.error['NM'] = fuzz.trimf(self.error.universe, [-2000, -1000, 0])
        self.error['NS'] = fuzz.trimf(self.error.universe, [-1000, 0, 1000])
        self.error['ZE'] = fuzz.trimf(self.error.universe, [-500, 0, 500])
        self.error['PS'] = fuzz.trimf(self.error.universe, [0, 1000, 2000])
        self.error['PM'] = fuzz.trimf(self.error.universe, [1000, 2000, 4000])
        self.error['PB'] = fuzz.trapmf(self.error.universe, [2000, 3000, 4000, 4000])

        # Funciones de pertenencia para 'delta_error'
        self.delta_error['NB'] = fuzz.trapmf(self.delta_error.universe, [-2000, -2000, -1000, -500])
        self.delta_error['NM'] = fuzz.trimf(self.delta_error.universe, [-1000, -500, 0])
        self.delta_error['NS'] = fuzz.trimf(self.delta_error.universe, [-500, 0, 500])
        self.delta_error['ZE'] = fuzz.trimf(self.delta_error.universe, [-250, 0, 250])
        self.delta_error['PS'] = fuzz.trimf(self.delta_error.universe, [0, 500, 1000])
        self.delta_error['PM'] = fuzz.trimf(self.delta_error.universe, [500, 1000, 2000])
        self.delta_error['PB'] = fuzz.trapmf(self.delta_error.universe, [1000, 1500, 2000, 2000])

        # Funciones de pertenencia para 'output' ajustadas para 0 a 4000 RPM
        self.output['NB'] = fuzz.trapmf(self.output.universe, [0, 0, 475, 975])
        self.output['NM'] = fuzz.trimf(self.output.universe, [475, 975, 1470])
        self.output['NS'] = fuzz.trimf(self.output.universe, [975, 1470, 2070])
        self.output['ZE'] = fuzz.trimf(self.output.universe, [1470, 2070, 3500])
        self.output['PS'] = fuzz.trimf(self.output.universe, [2070, 3500, 4000])
        self.output['PM'] = fuzz.trimf(self.output.universe, [3500, 3750, 4000])
        self.output['PB'] = fuzz.trapmf(self.output.universe, [3750, 3900, 4000, 4000])
    ...
    def update(self, vc):
        ...
        delta_error = error - self.last_error if hasattr(self, 'last_error') else 0
...
class AnimationPlot:
    ...
    def getPlotFormat(self):
        ...
        ax.set_title("Gráfica PID Difuso para controlar RPM")
